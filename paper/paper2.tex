...
%% Classification techniques in the Process Mining field have already been explored~\cite{DBLP:conf/bpm/RozinatA06}\cite{DBLP:conf/sac/LeoniA13}. In~\cite{DBLP:conf/bpm/RozinatA06} the idea is exploring how data influences the case routing of process flow execution. In this contribution the goal is to strengthen the use of data analysis techniques for the benefit of process analysis. This is done by studying how data may influence, in general, the overall process behavior.
...
%Note that the Petri net of Figure~\ref{pnet} is obtained by transforming a BPMN model using the algorithm described in~\cite{DBLP:conf/wsfm/BruniCFFGS11}, and mapping each activity to a pair of start/end transitions. The resulting net has been simplified by removing the unnecessary \emph{invisible transitions} used to encode e.g. the \emph{Join} or \emph{Fork} gateway~\cite{DBLP:conf/wsfm/BruniCFFGS11}.
...
%in gen pair $e = (a,t)$ representing an action $a$ recorded and the corresponding timestamp $t$.  
Events that belong to the same process instance are grouped in a {\itshape trace}, where events are ordered 
% A trace $T$ is a finite sequence of events ordered 
...
%DBLP:journals/widm/AalstAD12,
%,DBLP:conf/bpm/Munoz-GamaC10
...
%% sequence on a trace can be represented as an ordered list $R$ of pairs $(tr, i)$, representing that the transition $tr$ has been fired to mimic the event $T[i]$.
The output of the log replay algorithm  can be used to check the conformance of the traces.
% and to evaluate some performance metrics.
...
% process
%instance provides an additional and important data, 
...
%To get more significant information for our analysis, 
